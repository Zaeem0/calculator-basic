...
class Calculator:
    ...
    def equal_button_press(self):
        if self.add_trigger or self.sub_trigger or self.mult_trigger or self.div_trigger:
            if self.isFloat(self.entry_value.get()):
                if self.add_trigger:
    ...
    def __init__(self, root):
        ...
        root.geometry("375x244")
        root.resizable(width=False, height=False)

        style = ttk.Style()
        style.configure("TButton", width=6, font="Serif 15", padding=10)
        style.configure("TEntry", font="Serif 18", padding=10)

        self.number_entry = ttk.Entry(root, textvariable=self.entry_value, width=58)
